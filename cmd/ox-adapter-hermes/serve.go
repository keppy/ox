...
type hermesSessionState struct {
	sessionID string
	offset    int64
}

func handleServe(srv *adapterruntime.Server) {
	store := adapterruntime.NewSessionStore[hermesSessionState]()
	...
	srv.OnFindSession(func(ctx context.Context, p adapterprotocol.FindSessionParams) (*adapterprotocol.FindSessionResult, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("find Hermes session: %w", err)
		}
		store.Set(p.AgentID, hermesSessionState{
	...
	srv.OnReadFromOffset(func(ctx context.Context, p adapterprotocol.ReadFromOffsetParams) (*adapterprotocol.ReadFromOffsetResult, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("read Hermes session from offset: %w", err)
		}
		store.Set(p.AgentID, hermesSessionState{sessionID: state.sessionID, offset: result.NewOffset})
		return result, nil
	})
