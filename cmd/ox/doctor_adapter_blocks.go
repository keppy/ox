...
func scanAdapterBlocks(content string) adapterBlockFinding {
	...
	for _, m := range adapterBlockMarkers {
		...
		for _, lit := range []string{"AGENT_ENV=pi", "AGENT_ENV=amp", "AGENT_ENV=aider", "AGENT_ENV=codex", "AGENT_ENV=gemini", "AGENT_ENV=droid", "AGENT_ENV=opencode", "AGENT_ENV=goose", "AGENT_ENV=hermes"} {
