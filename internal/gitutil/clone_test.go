...
func TestValidateCloneURL(t *testing.T) {
	...
	tests := []struct {
		...
		{"bare windows drive path rejected in production", `C:\Users\me\repo.bare`, nil, false, true},
		{"bare windows drive path allowed for local tests", `C:\Users\me\repo.bare`, nil, true, false},
		{"windows drive path with forward slashes allowed for local tests", "C:/Users/me/repo.bare", nil, true, false},
		{"drive-relative windows path is not a local path", "C:repo.bare", nil, true, true},
