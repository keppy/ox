...
echo '{"protocol_version":1,"name":"test-discover","display_name":"Test","version":"0.1.0","type":"session","capabilities":["session_reader"]}'`
	testguard.WriteShellExecutable(t, dir, "ox-adapter-test-discover", script)
...
echo '{"protocol_version":0,"name":"old-proto","version":"0.1.0","type":"session"}'`
	testguard.WriteShellExecutable(t, dir, "ox-adapter-old-proto", script)
...
echo '{"protocol_version":1,"name":"priority-test","version":"1.0.0","type":"session"}'`
	testguard.WriteShellExecutable(t, highDir, "ox-adapter-priority-test", scriptHigh)

	scriptLow := `#!/bin/sh
echo '{"protocol_version":1,"name":"priority-test","version":"2.0.0","type":"session"}'`
	testguard.WriteShellExecutable(t, lowDir, "ox-adapter-priority-test", scriptLow)
...
func TestDiscoverExternalAdapters_IgnoresNonAdapterBinaries(t *testing.T) {
	...
	testguard.WriteShellExecutable(t, dir, "some-other-tool", "#!/bin/sh\necho hi")
...
echo '{"protocol_version":1,"name":"test-replace","display_name":"External","version":"2.0.0","type":"session","capabilities":["session_reader"]}'`
	testguard.WriteShellExecutable(t, dir, "ox-adapter-test-replace", script)
	t.Setenv("OX_ADAPTER_PATH", dir)
...
func TestAdapterDirs_ResolvesSymlinkedExecutable(t *testing.T) {
	...
	realDir := t.TempDir()
	realBinary := testguard.WriteShellExecutable(t, realDir, "ox", "#!/bin/sh\necho fake-ox")
...
case "$1" in
  ...
  detect) echo '{"detected":true,"reason":"test"}' ;;
	...
	testguard.WriteShellExecutable(t, dir, "ox-adapter-ext-detect", script)
	t.Setenv("OX_ADAPTER_PATH", dir)
...
echo '{"protocol_version":1,"name":"ext-get","display_name":"Ext","version":"0.1.0","type":"session","capabilities":["session_reader"]}'`
	testguard.WriteShellExecutable(t, dir, "ox-adapter-ext-get", script)
	t.Setenv("OX_ADAPTER_PATH", dir)
